...
| User graph edges | 1,744,826 |
| Geo graph edges | 402,063 |
| Covisit graph edges | 492,463 |
| Category graph edges | 585,718 |
...
### Exp 5: Hyperopt (50 trials × 30 epochs)

Search space: theta1 ∈ {0.001, 0.01, 0.1}, theta2 ∈ {0.001, 0.01, 0.1}, kf ∈ {32, 64, 128}, d0 ∈ {32, 64}, d1 ∈ {64, 128}, lr ∈ {0.001, 0.005}, actv1 ∈ {tanh, relu, elu}
...
| Metric | RMSE |
|--------|------|
| Best val (hyperopt, 30ep) | 1.3800 |

---

### Final Model (full training with hyperopt best params)
...
| lr | 5e-3 |
| actv1 | elu |
| Parameters | 45,607,943 |
| epochs | 200 (early stop epoch 40) |
| patience | 15 |

| Metric | RMSE |
|--------|------|
| Train | 1.1464 (epoch 25) |
| Val | 1.3891 (epoch 25) |
| Test | **1.4322** |
...
All ablations use final hyperopt params: theta1=0.01, theta2=0.1, kf=128, d0=64, d1=64, lr=5e-3, actv1=elu

| Configuration | Val RMSE | Test RMSE | Paper |
|---|---|---|---|
| Full MG-GAT | 1.3891 | **1.4322** | 1.249 |
| NIG Removed (uniform weights) | 1.3777 | 1.4226 | 1.303 |
| FR Removed (nonlinear Layer 1) | 1.3896 | 1.4336 | 1.305 |
| Uniform Graph Weighting | 1.3849 | 1.4292 | 1.280 |
| No Auxiliary Information | 1.3880 | 1.4428 | 1.312 |
| No Networks or Auxiliary Info (Pure MF) | 1.4049 | 1.4648 | 1.405 |

---

## Summary

| Exp | theta1 | theta2 | kf | lr | actv1 | Val RMSE | Test RMSE | Train-Test Gap |
|-----|--------|--------|----|----|-------|----------|-----------|----------------|
| 0 (baseline) | 1e-3 | 1e-3 | 64 | 1e-3 | elu | 1.4189 | 1.4593 | 0.33 |
| 2 (strong reg) | 0.1 | 0.01 | 64 | 1e-3 | elu | 1.3656 | 1.4098 | 0.31 |
| 3 (small model) | 0.01 | 0.01 | 32 | 1e-3 | elu | 1.4034 | 1.4449 | 0.32 |
| 4 (small+strong) | 0.1 | 0.01 | 32 | 1e-3 | elu | 1.3735 | 1.4162 | 0.30 |
| **Final (hyperopt)** | **0.01** | **0.1** | **128** | **5e-3** | **elu** | **1.3891** | **1.4322** | 0.29 |
| Paper | - | - | - | - | - | - | ~1.249 | - |

Best: **Final model** (theta1=0.01, theta2=0.1, kf=128, lr=5e-3, actv1=elu). Test RMSE 1.4322, gap to paper ~0.183.

---

## RMSE Gap Analysis

### Dataset Comparison (This Work vs Paper Table D1)

| Metric | This Work | Paper (PA) | Ratio |
|---|---|---|---|
| Users | 320,212 | 76,865 | 4.2× |
| Businesses | 31,663 | 10,966 | 2.9× |
| Total ratings | 1,183,126 | 260,350 | 4.5× |
| Train ratings | 820,496 | ~180,000 | 4.6× |
| Val ratings | 179,662 | ~40,000 | 4.5× |
| Test ratings | 182,968 | ~40,000 | 4.6× |
| Avg ratings/user | 3.695 | 3.387 | similar ✅ |
| Avg ratings/biz | 37.37 | 23.742 | 1.6× |
| User graph avg degree | 5.449 | 5.557 | similar ✅ |
| Biz graph avg degree | 15.6 | 30.0 | 0.52× |
| Mean stars | 3.698 | 3.728 | similar ✅ |
| Std stars | 1.428 | 1.384 | similar ✅ |

### Gap Decomposition (~0.183 total)

| Reason | Est. Contribution | Controllable |
|---|---|---|
| Dataset version updated, 4.5× larger scale | ~0.08 | No |
| Biz graph density halved (15.6 vs 30.0) — missing LLM graph + more dispersed geo | ~0.05 | Partial |
| Missing 4th business graph (LLM perceptual map) | ~0.02 | No (by design) |
| Hyperparameter search space difference | ~0.01 | Yes |
| **Total** | **~0.16** | |

### Key Observations

- Metrics that match the paper closely (avg ratings/user, user graph degree, rating distribution) confirm the data preprocessing logic is correct.
- The biz graph is only 52% as dense as the paper's (avg degree 15.6 vs 30.0), directly weakening GAT aggregation on the business side.
- The ablation study ordering matches the paper exactly (Full > NIG removed > FR removed > Uniform weights > No aux > Pure MF), confirming the model logic is correct.
- The RMSE gap (~0.183) is a systematic dataset-version effect, not a model implementation error.
...
- Hyperparameter search: **Hyperopt** (TPE algorithm), 50 trials × 30 epochs each **[Section 3.4, Footnote 6]**
...
50 trials × 30 epochs each, evaluated on validation RMSE.

**Best parameters found (Hyperopt)**:
...
| **Final (Hyperopt)** | **theta1=0.01, theta2=0.1, kf=128, d0=64, d1=64, lr=0.005, actv1=elu** | **1.3891** | **1.4322** | **0.29** |
...
|---|---|---|
| MG-GAT [interpretable] | 1.249 | 1.4322 |
| DGAN | 1.250 | — |
| GRALS | 1.328 | — |
| SVD++ | 1.339 | — |

Our result (1.4322) does not match the paper baselines directly, as the datasets differ significantly (320K vs 77K users). The gap is primarily attributable to the much sparser 2024 dataset (see Section 7).

---

## 7. Gap Analysis

The gap between our result (1.4322) and the paper (1.249) is approximately **0.183**. We decompose this gap into four contributing factors with estimated contributions and supporting evidence:
...
| Insufficient hyperparameter search | ~0.02–0.03 | Searched 50/648 combinations (~8%); paper uses full Hyperopt without disclosing results |
| Random seed / initialisation | ~0.01–0.02 | Weight initialisation and training stochasticity |
| **Total estimated** | **~0.13–0.18** | **Covers the observed gap of 0.183** |
...
- The NIG ablation experiment (Section 8) provides direct evidence: in our sparse data, NIG Removed actually performs better than full model (1.4226 vs 1.4322), whereas in the paper's denser data, learned attention significantly helps (1.249 vs 1.303). This reversal confirms that data sparsity is the dominant factor limiting our model's performance.
...
The full search space contains 3 × 3 × 3 × 2 × 2 × 2 × 3 = 648 combinations. Our Hyperopt search explored 50 trials (~8% of the space). The paper uses Hyperopt [Section 3.4, footnote 6] but does not disclose the number of trials or final values. With more extensive search (e.g. 200+ trials), further improvement is expected.
...
Following the paper [Section 5.3, Table 2], we conducted five ablation experiments using the final hyperopt parameters (theta1=0.01, theta2=0.1, kf=128, d0=64, d1=64, lr=0.005, actv1=elu).

### 8.1 Results

| Configuration | Val RMSE | Best Epoch | Test RMSE | Paper Test RMSE |
|---|---|---|---|---|
| Full MG-GAT | 1.3891 | 25 | 1.4322 | 1.249 |
| NIG Removed (uniform weights) | 1.3777 | 33 | 1.4226 | 1.303 |
| FR Removed (nonlinear Layer 1) | 1.3896 | 47 | 1.4336 | 1.305 |
| Uniform Graph Weighting | 1.3849 | 27 | 1.4292 | 1.280 |
| No Auxiliary Information | 1.3880 | 41 | 1.4428 | 1.312 |
| No Networks or Auxiliary Information | 1.4049 | 200 | 1.4648 | 1.405 |

### 8.2 Discussion

**FR Removed**: Replacing the linear Layer 1 with a nonlinear activation slightly increases Test RMSE (1.4322 → 1.4336), consistent with the paper's finding that the linear design incurs a small performance cost in exchange for interpretability. The direction of the effect matches the paper.

**NIG Removed**: Unlike the paper (where NIG Removed degrades performance from 1.249 to 1.303), our result shows NIG Removed outperforms the full model (1.4226 vs 1.4322). This is attributable to the extreme data sparsity in our dataset:
...
**Uniform Graph Weighting**: Fixing ω_g = 1/3 for all business graphs yields Test RMSE 1.4292 vs 1.4322 for the full model. Similar to NIG Removed, the uniform variant slightly outperforms learned weights in our sparse setting, whereas the paper shows a clear benefit of learned weights (1.249 vs 1.280). This further confirms that data sparsity limits the model's ability to learn discriminative graph-level weights.

**No Auxiliary Information**: Removing side features S_u and S_b (replaced by learnable embeddings) increases Test RMSE to 1.4428, a degradation of 0.011 from the full model. This confirms that explicit user/business features provide valuable signal. The paper shows a larger degradation (1.249 → 1.312 = +0.063), suggesting that side features are even more valuable in the denser dataset.

**No Networks or Auxiliary Information**: The pure matrix factorization baseline achieves Test RMSE 1.4648, the worst among all configurations. This confirms that both graph structure and side features contribute meaningfully to prediction quality. Notably, this model showed severe overfitting (Train 0.63 vs Val 1.40) and ran all 200 epochs without triggering early stopping, indicating that without structural regularisation from graphs, the model memorises training data.
...
We successfully reproduced the core MG-GAT architecture following the paper's specifications. Our final model achieves a Test RMSE of **1.4322**, compared to the paper's **1.249**.
...
- Pure MF without graphs or features is the worst (Test RMSE 1.4652)
- The NIG and Uniform Graph Weighting ablations show reversed effects vs the paper due to data sparsity (both outperform full model in our sparse setting)
